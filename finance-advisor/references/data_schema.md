...
| 数据源 | 存储位置 | 注册地址 |
|--------|----------|----------|
| TickDB | 不需要（自动获取试用Key） | https://tickdb.ai |
| Finnhub | `memory/finnhub-key.json` | https://finnhub.io/register |
| Twelve Data | `memory/twelvedata-key.json` | https://twelvedata.com |
| Yahoo Finance | 不需要（Python库，无需Key） | `pip install yfinance` |
...
**twelvedata-key.json 格式**:
```json
{"api_key": "your_twelvedata_key_here"}
```
